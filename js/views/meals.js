...
import { listStepsForMeals, slugifyFoodName } from '../data/mealSteps.js';
import { createMethodSection } from './meals/method.js';
...
export function render(mountEl) {
  ...
  // ---- Worklist G1: ingredient builders live in their own module ----
  // See js/views/meals/ingredients.js. It does not fetch and does not
  // cache: macros, method.js and cookNow.js all read these rows, so they
  // keep ONE owner here and everything else is handed them.
  //
  // Rebuilt on every food load rather than held, because `foods` changes
  // and a builder closed over a stale list would offer foods that are gone.
  let ingredients = null;
  function rebuildIngredientBuilders() {
    ingredients = createIngredientBuilders({
      foods,
      signal,
    ...
    if (rows.length > 0) {
      ...
      for (const entry of groupIngredientOptions(rows)) {
        list.appendChild(entry.kind === 'group'
          ? ingredients.buildOptionGroupRow(meal, entry)
          : ingredients.buildIngredientRow(meal, entry.row));
      }
      body.appendChild(list);
    }

    body.appendChild(ingredients.buildAddIngredientForm(meal));
    // ---- Worklist G1: the method lives in its own module ----
    // See js/views/meals/method.js. It is given the ingredient rows rather
    // than fetching them: a step's {{ing:}} token resolves against them,
    // and they have one owner, which is not that module.
    body.appendChild(createMethodSection({
      meal,
      steps: stepsByMeal.get(meal.id) || [],
      ingredientRows: rows,
      pantryStock,
      signal,
      isDestroyed: () => destroyed,
  ...
  async function loadFoods() {
    ...
    foods = result.data;
    // The builders close over `foods`, so they are rebuilt whenever it
    // changes. A builder holding a stale list would offer foods that have
    // since been deleted.
    rebuildIngredientBuilders();
    renderMeals();
